...
export class RiderAgentController {
  ...
  @ApiOperation({ summary: 'Create a new rider', description: 'Registers a new rider in the system (SaaS multi-tenant aware)' })
  @ApiResponse({ status: 201, description: 'Rider created successfully', type: RiderResponseDto })
  createRider(@Req() req: any, @Body() createRiderDto: CreateRiderDto): any {
  ...
  @ApiOperation({ summary: 'Update rider location', description: 'Updates the GPS coordinates of a specific rider' })
  @ApiResponse({ status: 200, description: 'Location updated successfully', type: RiderLocationResponseDto })
  updateLocation(
    @Param('id') id: string,
