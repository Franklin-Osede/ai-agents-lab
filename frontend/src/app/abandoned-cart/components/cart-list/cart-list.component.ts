...
export class CartListComponent implements OnInit {
  ...
  carts = computed(() => {
    ...
    if (search) {
      filtered = filtered.filter(cart => {
        ...
        const productNames = cart.items.map(item => item.name.toLowerCase()).join(' ');
        return name.includes(search) || email.includes(search) || company.includes(search) || productNames.includes(search);
      });
    }

    // Status filter
    if (this.statusFilter() !== 'all') {
      filtered = filtered.filter(cart => cart.status === this.statusFilter() as CartStatus);
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (this.sortBy()) {
        case 'value':
          ...
          break;
        case 'date':
          comparison = new Date(a.lastModifiedAt).getTime() - new Date(b.lastModifiedAt).getTime();
          break;
        case 'probability':
          comparison = (a.recoveryProbability || 0) - (b.recoveryProbability || 0);
          break;
      }
  ...
  recoverCart(cartId: string): void {
    ...
    const phone = cart.customer?.phoneNumber?.replace(/[^0-9]/g, '');
    const message = encodeURIComponent(`Hola ${cart.customer?.name || ''}, notamos que dejaste productos en tu carrito. ¿Te ayudo a completarlo?`);
    if (phone) {
      window.open(`https://wa.me/${phone}?text=${message}`, '_blank');
    }
    carts[idx] = { ...cart, status: CartStatus.RECOVERED, recoveryAttempts: (cart.recoveryAttempts || 0) + 1 };
  ...
  getCustomerAvatar(cart: Cart): string {
    ...
    const seed = cart.customerId || cart.id || Math.random().toString(36).substring(7);
  ...
  getMockNotifications(): { id: string; message: string; time: string; read: boolean }[] {
    return [
      ...
      { id: '3', message: '3 carritos necesitan seguimiento', time: 'Hace 2 horas', read: true },
    ];
  }
  ...
  loadMoreCarts(): void {
    ...
    setTimeout(() => {
      const extra = Array.from({ length: 3 }).map((_, i) => this.generateMockCart(i));
      this.allCarts.set(this.withRandomStatuses([...this.allCarts(), ...extra]));
  ...
  private generateMockCart(seed: number): Cart {
    ...
    const products = ['Mochila', 'Audífonos', 'Teclado Mecánico', 'Mouse Ergo', 'Lámpara LED', 'Monitor 27"'];
  ...
  private withRandomStatuses(carts: Cart[]): Cart[] {
    ...
    return carts.map((cart, idx) => ({
      ...
      status: (idx % 4 === 0 || idx % 5 === 0) ? CartStatus.RECOVERED : CartStatus.ABANDONED,
    }));
  }
