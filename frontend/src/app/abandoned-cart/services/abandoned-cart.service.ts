...
export class AbandonedCartService {
  ...
  private getMockCarts(): Cart[] {
    ...
    const names = ['María Gómez', 'Carlos Ruiz', 'Laura Torres', 'Pedro Sánchez', 'Ana López', 'David Díaz', 'Elena Morales', 'Pablo Herrera'];
    const products = ['Auriculares', 'Zapatillas', 'Bolso', 'Reloj', 'Cámara', 'Teclado', 'Monitor', 'Silla Ergo'];
    ...
    for (let i = 0; i < 18; i++) {
      ...
      carts.push({
        ...
        lastModifiedAt: new Date(Date.now() - Math.random() * 72 * 60 * 60 * 1000),
        recoveryAttempts: isRecovered ? 1 : 0,
        recoveryProbability: Math.floor(Math.random() * 100),
      });
      // Fix total price match
      carts[carts.length - 1].items[0].totalPrice = carts[carts.length - 1].items[0].unitPrice;
      carts[carts.length - 1].totalValue = carts[carts.length - 1].items[0].unitPrice;
    }

    return carts.sort((a, b) => b.lastModifiedAt.getTime() - a.lastModifiedAt.getTime());
  }
